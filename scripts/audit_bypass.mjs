...
(async () => {
    ...
    // Set localStorage to bypass onboarding completely
    await page.evaluate(() => {
        localStorage.setItem('onboardingStep', '5'); // ONBOARDING_GUIDED_COMPLETE
        localStorage.setItem('seen_demo_welcome', 'true');
    ...
    // At this point it should jump directly to the Stream since onboarding is skipped
    // If it still shows DemoWelcomeModal, we can try to close it if it's there
    await page.evaluate(() => {
        const btns = Array.from(document.querySelectorAll('button'));
        const close = btns.find(b => b.textContent.includes('Start Exploring') || b.textContent.includes('Got it'));
    ...
    await page.evaluate(() => {
        const btns = Array.from(document.querySelectorAll('button, a, div[role="tab"]'));
        const tab = btns.find(b => b.textContent.includes('Stream') || b.textContent.includes('Journal'));
        if (tab) tab.click();
    }).catch(e=>console.log(e));
