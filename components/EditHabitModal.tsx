          ...
          <div className="flex gap-2 overflow-x-auto pb-2 scrollbar-thin">
            {emojis.map(e => (
              <button
                key={e}
                onClick={() => setEmoji(e)}
                className={`w-10 h-10 flex-shrink-0 rounded-lg text-xl flex items-center justify-center transition-colors ${emoji === e ? 'bg-brand-teal text-white' : 'bg-dark-surface-light text-white hover:bg-white/10'}`}
              >
                {e}
              </button>
            ))}
